...
IMG_PATH = r"C:\tortoisegit_opencv\lenna.bmp"
...
def sobel_derivative():
    ...
    dx = cv2.filter2D(src, -1, mx, borderType=cv2.BORDER_DEFAULT)
    dy = cv2.filter2D(src, -1, my, borderType=cv2.BORDER_DEFAULT)
...
def sobel_edge():
    ...
    edge = (mag > 50).astype(np.uint8) * 255
